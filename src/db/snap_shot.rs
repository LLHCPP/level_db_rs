...
impl SnapshotList{
    ...
    fn New(&mut self, sequence_number:u64)-> *mut  SnapshotImpl{
        ...
        unsafe { (*snapshot).next_ = &mut self.head; }
        unsafe { (*snapshot).prev_ = self.head.prev_; }
        unsafe { (*(*snapshot).prev_).next_ = snapshot; }
        unsafe { (*(*snapshot).next_).prev_ = snapshot; }
        snapshot
    }
    
    
    fn delete(&mut self, snapshot: *mut SnapshotImpl) {
        unsafe { (*(*snapshot).prev_).next_ =(* snapshot).next_; }
        unsafe { (*(*snapshot).next_).prev_ =(* snapshot).prev_; }
        unsafe { let _ = Box::from_raw(snapshot); }
    }
